...
public abstract class Invoker<S extends BasicSource<I, ?, Void>, I, O> {
	...
	/**
	 * Return a string to use as the attribute type when saving to ARFF format.
	 * See https://weka.wikispaces.com/ARFF+%28book+version%29
	 * 
	 * @return
	 */
	...
	public void invoke(List<S> sources, List<I> inputs) {
		...
		// create a 2d array or results where rows are inputs and columns are
		// sources
	 ...
	 *            - an open file to write to
	 * @param comment
	 *            - A string to include as a comment at the top of the file.
	...
	public void saveToArff(File destination, String comment) throws IOException {
		...
		for (int i = 0; i < numSources(); i++) {
			buf.write(String.format("@attribute %s %s\n", getSource(i).getName().replace(" ", "-"), attributeType));
		}
		buf.write("\n");
	...
	/**
	 * Returns a string which can be used as the header for a file containing
	 * the results of the invocation. Contains the date and time of creation.
	 */
	...
	/**
	 * Returns a weka dataset in the form of an Instances object. By convention,
	 * the class attribute is at index 0
	 * 
	 * @param useStringValues
	 *            - Only strings or doubles are accepted as values in a weka
	 *            instance. If set to false then doubles will be used. The
	 *            results stored in this object will be cast and then supplied
	 *            to a weka instance.
	 * @return
	 * @throws Exception
	 *             - throws an exception if the sources stored in this object
	 *             have changed since they were last invoked to generate
	 *             results.
	 */
	public Instances getResultInstances(boolean useStringValues) throws Exception {
		...
		ArrayList<Attribute> attributes = new ArrayList<Attribute>(sources.size());
		...
		for (int i = 0; i < results.length; i++) {
			Instance instance = new DenseInstance(sources.size());
			instance.setDataset(instances);
			for (int j = 0; j < sources.size(); j++) {
				// cast the results as necessary
				if (useStringValues) {
					instance.setValue(j, String.valueOf(results[i][j]));
				} else {
					instance.setValue(j, Double.parseDouble(String.valueOf(results[i][j])));
				}
	...
	/**
	 * This method is used when an invocation was done which results in a 2d
	 * array. The sources are separated by commas and the events are separated
	 * by newlines.
	 */
