...
/**
 * Checks that the event organizer's phone number area code matches the location
 * of the event.
 * 
 * @author wginsberg
 *
...
public class AreaCodeValid extends EventSource {
	...
	protected Integer getResponseOnline(Event e) throws UnknownException {
		...
		if (e.getOrganizer() == null || e.getOrganizer().getContactInfo() == null
				|| e.getOrganizer().getContactInfo().getPhoneNumber() == null) {
			return -1;
		}
		...
		if (e.getVenue() == null || e.getVenue().getAddress() == null || e.getVenue().getAddress().getCity() == null) {
			System.out.println("Null venue/address/city: " + e);
			return -1;
		}

		// do a coocurrence search for the location of the event and the area
		// code
